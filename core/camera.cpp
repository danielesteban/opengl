...
Camera::Camera() {
  position = glm::vec3(0.0f, 2.5f, 0.0f);
  yaw = glm::radians(270.0f);
  pitch = 0.0f;
  updateVectors();
}

void Camera::animate(Input *input, GLfloat time, GLfloat delta) {
...
bool Camera::isInFrustum(glm::vec3 origin, GLfloat radius) {
  for (int i = 0; i < 6; i += 1) {
    GLfloat distance = glm::dot(frustum[i].normal, origin) + frustum[i].constant;
    if (distance < -radius) {
      return false;
    }
  }
...
void Camera::updateFrustumPlane(GLuint index, GLfloat x, GLfloat y, GLfloat z, GLfloat w) {
